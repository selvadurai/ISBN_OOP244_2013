...
int oTests(ISBNPrefix& list, int& testNumber, int* totalPassed, int* totalTested) {
   ...
   //
	...
	char testName[100];
	// Order(ISBN(1999999908,"John Smith")
    order = Order(ISBN("1999999908", list),"John Smith");
    order.name(testName);
    if(order.has(ISBN("1999999908", list)) == 0 || order.outstanding() != 0 ||strcmp(testName,"John Smith")!=0) {
        testFailed("has(ISBN(\"1999999908\", )), outstanding(),name(char*)",
         ++testNumber, &ok, totalTested);
    } else
        testPassed("has(\"1999999908\"), outstanding(), name(char*)", ++testNumber, totalPassed,
         totalTested);

	// Order(ISBN(1999999908),NULL)
    order = Order(ISBN("1999999908", list),NULL);
    if(order.has(ISBN()) == false) {
        testFailed("Order(ISBN(1999999908),NULL), has(ISBN()) did not return true",
         ++testNumber, &ok, totalTested);
    } else
        testPassed("Order(ISBN(1999999908),NULL), has(ISBN()) returned true", ++testNumber, totalPassed,
         totalTested);
    ...
    //
    if (DEBUG)
        printf("\nOrder Assignment Operator & Copy Constructor Tests\n"
               "===========================================================\n");
    Order order0;
    Order order1(ISBN("9070002043", list), "James Harvey");
    order1.add(10);
    Order order3(ISBN("4200000008", list), "John Smith");
    order3.add(5);
    Order *orderptr;
    order0 = order1;
    if(order0.has(ISBN("9070002043", list))==false || order0.outstanding() != 10) {
        testFailed("has(\"9070002043\"), outstanding()", ++testNumber,
            &ok, totalTested);
    } else
    ...
    Order order2= order3;
    order2.name(testName);
    if(order2.has(ISBN("4200000008", list))==false || order2.outstanding() != 5 ) {
        testFailed("has(\"4200000008\"), outstanding()", ++testNumber,
            &ok, totalTested);
    } else
        testPassed("has(\"4200000008\")), outstanding()", ++testNumber,
            totalPassed, totalTested);

    printf("\ndestructor and deep copy check\n");
    orderptr=new Order(ISBN("1999999908",list), "Kimberly Jones");
    order1=*orderptr;
    delete orderptr;
    order1.name(testName);
    if(order1.has(ISBN("1999999908", list))==false || order1.outstanding() != 0||strcmp(testName,"Kimberly Jones")!=0) {
        testFailed("has(\"1999999908\"), outstanding(), name()", ++testNumber,
            &ok, totalTested);
    } else
        testPassed("has(\"1999999908\")), outstanding(), name()", ++testNumber,
            totalPassed, totalTested);
