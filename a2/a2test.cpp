...
*   - added check for valid ISBN *X (but isValid returns false)
*
...
int rTests(ISBNPrefix& list, int& testNumber, int* totalPassed, int* totalTested) {
    ...
    for (int i = 0; i < total_isbn; i++) {
	  ...
	  // All other ISBNs should be true - check to make sure
	  } else if (isValid(value[i]) == 0) {
	    sprintf(message, "       isValid(...) returned "
		 "false  for %s", value[i]);
	    testFailed(message, ++testNumber, &ok, totalTested);
